...
<div class="share-card-wrapper w-full max-w-[360px] rounded-xl p-3 shadow-2xl {themeClasses()}">
	<!-- Header Section -->
	<div class="mb-2">
		<div class="mb-2">
			<div class="mb-2 flex items-center gap-2">
				<Server class="h-3 w-3 shrink-0 opacity-70" />
				<h2 class="flex-1 truncate text-base font-bold">{server.name}</h2>
			</div>
			<!-- Badges: Map, Mode, Capacity -->
			<div class="flex flex-wrap gap-2">
				<!-- Map Badge -->
				<span
					class="badge badge-outline rounded-md border-cyan-200 bg-cyan-50 px-2 py-1 text-[10px] font-medium text-cyan-700 shadow-sm"
				>
					{server.mapId.split('/').pop() || server.mapId}
				</span>
				<!-- Mode Badge -->
				<span
					class="badge badge-outline rounded-md border-blue-200 bg-blue-50 px-2 py-1 text-[10px] font-medium text-blue-700 shadow-sm"
				>
					{server.mode || 'Unknown'}
				</span>
				<!-- Capacity Badge -->
				<span
					class="badge badge-success rounded-md px-2 py-1 text-[10px] font-medium text-white shadow-sm"
				>
		...
		{#each serverInfoFields as field}
			{#if isFieldVisible(field.key)}
				<div class="flex items-center justify-between gap-1">
					...
					<span
						class="truncate text-right text-[10px] font-semibold {field.isMono ? 'font-mono' : ''}"
					>
						{field.format
		...
		{#if server.url && isFieldVisible('url')}
			<div class="flex items-center justify-between gap-1">
				...
				<a
					...
					class="link link-primary truncate text-[10px]"
					title={server.url}
				>
		...
		<div class="border-base-content/15 mt-2 border-t pt-2">
			...
			<div class="flex flex-wrap gap-1">
				{#each server.playerList as player}
					<span class="badge badge-neutral flex-shrink-0 text-[10px] whitespace-nowrap"
						>{player}</span
					>
				{/each}
	...
	<div class="border-base-content/15 mt-2 border-t pt-2">
		...
		{#if showWatermark && watermarkText}
			<div class="text-base-content/30 mt-1 text-center text-[9px]">{watermarkText}</div>
		{/if}
	</div>
